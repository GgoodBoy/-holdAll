    ...
    <view class="audio-pluge">    
        <view class="audio-box">
            <view class="copy_audio">
                <image webp lazy-load :src="objs.surfacePlot" @touchmove='touchmoveEven($event,1)' @touchstart="touchstartEven($event,1)" @touchend='touchendEven($event,1)'/>
            ...
            <view class="textCourse">
                <view style="padding: 30rpx;background:#fff;">
                    ...
                    <view class="userBox">
                        ...
                        <!-- <view class="moneyBut" @click="purchaseCourse">
                        ...
                        <!-- timeLimitType: 0免费1打折2秒杀-1非限时购课程 -->
                        ...
                        <view class="spellList" v-for="(item,index) in spellTableData2" :key='index'>
                            <view class="spellLefts">
                                <view class="userBox" style="width:160rpx">
                                    <image class="users" :src="item1" :style="{'left':index1*40+'rpx'}" v-for="(item1,index1) in item.headUrlList" :key="index1+'u'" webp lazy-load />
                                </view>
...
export default {
    ...
    props:['selectMoneyData','xgxwChannelData','loginType','userCellphoneType','objs','periodObg','imgUrls','changeDurationNew','versionType','datalist','limitObj','timeHideOrShow','iosOrAndroid','changeDataMoney','courseIsSpellStates'],
    components:{
                            ...
                            self.$http.post(self.$server.savePeriodStudyRecordsUser,query).then(res=>{
                                if(res.data.status==200){
                                    ...
                                    setTimeout(()=>{
                                        ...
                                        //判断是不是7天，30天价格的设置课程
                                        if(self.propsData.type==1||self.propsData.type==2){
                                            wx.redirectTo({
                                                url: '../investMoney/index?id='+Number(self.objs.courseId)+"&timeLimitType="+self.objs.timeLimitType+"&spikePrice="+moneySpikePrice+"&isJoinAssemble="+self.objs.isJoinAssemble+"&assembleCourseId="+self.objs.assembleCourseId+"&isSpellCourseBuy="+self.isSpellCourseBuy+"&selectSpellListId="+self.selectSpellListId+"&xgxwType="+self.propsData.type
                                            });
                                        }else{
            ...
            self.$http.post(self.$server.getJoinAssembleCourseInfo,query,{urlType:false}).then(resInfo=>{
                if(resInfo.data.status==200){ 
                    if(Num==2){
                        ...
                        console.log("this.spellTableData2",this.spellTableData2);
                    }else{
