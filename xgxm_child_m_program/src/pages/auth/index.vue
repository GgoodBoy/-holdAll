<template>
    <view class="auth-page" :style="{paddingTop:lowerVersion?'0':paddingTop+'px'}">
        <Header :title="'学国学网登录'" :returnBools="true" :userPageType="userPageType" :isNoUserAndSearch="true"></Header>
        <view class="content">
            <view class="btn">
...
export default {
    ...
    onLoad(options){
        ...
        if(options.isUserType){
            this.userPageType = options.isUserType;
        }
    },
    onShow(){
        if(this.lowerVersion){
             wx.setStorageSync('authFirst',true);
        }
    },
    methods:{
        openUrl(type){
