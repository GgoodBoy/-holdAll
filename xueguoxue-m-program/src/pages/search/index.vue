<template>
    <view class="search-page" :style="{paddingTop:lowerVersion?'0':paddingTop+'px'}">
                ...
                <view class="result-list" :class="loading2?'loading':''">
                    ...
                    <List :loading="loading2" :text="text" :textFlag="true" :finished="finished" :height="height" @load="getResult">
                        <view class="result-item" v-for="(item,index) in resultList" :key="index">
                            ...
                            <view class="period-list" v-if="item.periodsList.length>0" :style="{height:item.height}">
...
export default {
    ...
    methods:{
        ...
        getResult(){
            ...
            this.$http.post(this.$server.searchWechatCourseList,query,{urlType:true}).then(res=>{
                this.loading2 = false;
                if(res.data.status==200){
