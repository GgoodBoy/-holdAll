        ...
        <el-container>
            <el-header>
                <el-row class="lLine40">
                    ...
                    <el-col v-show="closeBox===false?false:true" :span="2" :offset="18" @click.native="closeCourseList" class="lSize18-i">返回</el-col>
            ...
            <el-main>
                <el-row class="lMarginTB20" :gutter="10">
                    ...
                    <el-col :span="3" v-if="varObjs.mtypeText">  
                    ...
                    <el-col :span="4" v-if="varObjs.mtypeText">   
                        <el-button style="background-color:#9e0e00;color:#fff" @click="saveSeach">查询</el-button>
                    </el-col>                                        
                    <el-col :span="3" :offset="varObjs.mtypeText?3:13">已选择<span class="redFontColor">{{changeChooseIdlist.length}}</span>门课程</el-col>
                </el-row>
                <el-row>
                    <el-table
                        v-loading = "loadDailg"
                        ...
                        maxHeight="500" 
                        tooltip-effect="dark"
                        v-bind:class="{ cellTable: (isAllSelect&&isAllSelect.type==1)?false:true }"
                        style="width: 100%"
                        ...
                        show-overflow-tooltip>
                            <template slot-scope="scope">
                                <P>{{scope.row.sellingPrice||0}}币</P>
                            </template>                          
                        </el-table-column>
                    </el-table>                    
                </el-row>
...
export default {
    ...
    created(){
        this.getDataList();
        if(this.varObjs.mtypeText){
            this.getConstant();
        }
    },
    props:['varObjs','isAllSelect',"courseType","channelObj","numbersBool","urlPorp","chooseIdlist","relevanceId","locaCourseIds",'closeBox'],
    computed:{},
    methods:{
        getConstant() {
            this.$http
      ...
      handleSelectionChange(val,data) {
        ...
        saveSeach(){
            ...
            if(self.courseType=="course"){
                ...
                // self.changeChooseIdlist = courseIds;
                self.$http
        ...
        ajaxData(){
            ...
            var getAppInfo= function (){
                    ...
                    }else if(self.courseType=='channel'){
                        var urls='',
                            id = self.channelObj.selectChannelId;
                        if(id){
                            urls = self.urlPorp+"?courseName="+self.input2+"&id="+id;
                        }else{
                            urls = self.urlPorp+"?courseName="+self.input2;
                        }
                        self.$http.get(urls)
                                    .then(res => {
                                        if (res.status === 200) {
        ...
        watch:{
            ...
            multipleSelection(curVal,oldVal){
                var self = this;
                if(self.inputBoos){ 
                    self.inputBoos = false;
                }
                console.log("curVal",curVal.length);
            }
        },
    mounted() {
...
.cellTable .has-gutter th.el-table-column--selection .cell{
    display: none;
}
</style>
