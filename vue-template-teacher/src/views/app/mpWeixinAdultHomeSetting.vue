    ...
    <div class="adult-home-setting clearfix">
                ...
                <div v-for="(item,index) in homeEditData" :key="index" class="item" :class="item.active?'active':''" @click="switchTab(item)">
                    <el-dropdown size="small" placement="bottom-start">
                        <p class="el-dropdown-link">
                            <i class="el-icon-sort prev" v-if="$authJudge('homeset:appletadult:sort')" v-show="homeEditData.length>1&&index>0" @click.stop="cloumnSort(item,'prev')"></i>
                            <span>{{item.tabName}}</span>
                            <i class="el-icon-sort next" v-if="$authJudge('homeset:appletadult:sort')" v-show="homeEditData.length>1&&index<homeEditData.length-1" @click.stop="cloumnSort(item,'next')"></i>
                        </p>
                        <el-dropdown-menu slot="dropdown" v-if="$authJudge('homeset:appletadult:delcolumn')||$authJudge('homeset:appletadult:editcolumn')">
                            <el-dropdown-item v-if="$authJudge('homeset:appletadult:delcolumn')" @click.native="delColumn(item,index)">删除</el-dropdown-item>
                            <el-dropdown-item v-if="$authJudge('homeset:appletadult:editcolumn')" @click.native="openCloumnNameDialog(item)">修改</el-dropdown-item>
                        </el-dropdown-menu>  
                    </el-dropdown>
                </div>
            </div>
            <div class="add" v-if="$authJudge('homeset:appletadult:addcolumn')">
        ...
        <div class="left-box" :class="focusFlag?'in-edit':''">
            <div class="mobile" ref="view" id="wrapper">
                <div class="scroll" ref="scroll">
                   <components v-for="(item,index) in curHomeEditData" :key="index" :is="item.moduleName" :moduleData="item" ref="myModule" @click.native="chooseModule(item,index)" @mousedown.native="myMousedown($event)" @mouseup.native="myMouseup" ></components>
                </div>
            </div>        
            <div class="module-edit-btns">
                <p v-if="$authJudge('homeset:appletadult:upmodule')" @click="sortUp" :class="moveUp?'allow-move-up':''"><i class="el-icon-caret-top"></i><span>上移</span></p>
                <p v-if="$authJudge('homeset:appletadult:downmodule')" @click="sortBottom" :class="moveDown?'allow-move-down':''"><i class="el-icon-caret-bottom"></i><span>下移</span></p>
                <p v-if="$authJudge('homeset:appletadult:delmodule')" @click="delModule"><i class="el-icon-delete"></i><span>删除</span></p>
            </div>
            <div class="btns" v-if="curHomeEditData.length>0||delLastFlag">
                <div v-if="focusFlag||delLastFlag">
                    <el-button v-if="$authJudge('homeset:appletadult:savemodule')" size="medium" type="primary" @click="saveAppHomeData">保存</el-button>
        ...
        <div class="right-box handle">
            <transition mode="out-in">
                <component :is="curEdit" v-if="curEdit!=''" :settingType="'3'"></component>
            </transition>
        </div>
...
import Banner from './components/adultComponents/banner'//banner展示
import BannerEdit from './components/adultComponents/bannerEdit'//banner编辑
import ClassifyNav from './components/adultComponents/classifyNav'//分类\标签展示
import ClassifyNavEdit from './components/adultComponents/classifyNavEdit'//分类\标签编辑
import Advertising from './components/adultComponents/advertising'//通栏广告展示
import AdvertisingEdit from './components/adultComponents/advertisingEdit'//通蓝广告编辑
import ColumnsCourse from './components/adultComponents/columnsCourse'//通栏课展示
import ColumnsCourseEdit from './components/adultComponents/columnsCourseEdit'//通栏课编辑
import GuessLike from './components/adultComponents/guessLike'//猜你喜欢
import GuessLikeEdit from './components/adultComponents/guessLikeEdit'//猜你喜欢编辑
import Customize from './components/adultComponents/customize'//自定义
import CustomizeEdit from './components/adultComponents/customizeEdit'//自定义编辑
import TimeZone from './components/adultComponents/timeZone'//限时专区
// import TimeZoneEdit from './components/adultComponents/timeZoneEdit'//限时专区编辑
import TimeZoneEdit from './components/adultComponents/timeZoneNextEdit'//限时专区编辑
import FreeZone from './components/adultComponents/freeZone'//免费专区
import FreeZoneEdit from './components/adultComponents/freeZoneEdit'//免费专区编辑
import PopularCourses from './components/adultComponents/popularCourses'//热门课程
import PopularCoursesEdit from './components/adultComponents/popularCoursesEdit'//热门课程编辑
import Consult from './components/adultComponents/consult'//咨询
import ConsultEdit from './components/adultComponents/consultEdit'//咨询编辑
import Special from './components/adultComponents/special'//专题
import SpecialEdit from './components/adultComponents/specialEdit'//专题编辑
import Classify from './components/adultComponents/classify'//分类
import ClassifyEdit from './components/adultComponents/classifyEdit'//分类编辑
import Tag from './components/adultComponents/tag'//标签
import TagEdit from './components/adultComponents/tagEdit'//标签编辑
import Knowledge from './components/adultComponents/knowledge'//知识点(短视频)
import KnowledgeEdit from './components/adultComponents/knowledgeEdit'//知识点编辑(短视频)
import Assemble from './components/adultComponents/assemble'//拼团专区
import AssembleEdit from './components/adultComponents/assembleEdit'//拼团专区编辑
import Live from './components/adultComponents/live'//拼团专区
import LiveEdit from './components/adultComponents/liveEdit'//拼团专区编辑
...
const subModule = [
    {type:1,val:'banner'},{type:2,val:'classifyNav'},{type:3,val:'advertising'},{type:4,val:'guessLike'},{type:5,val:'columnsCourse'},
    {type:6,val:'customize'},{type:7,val:'timeZone'},{type:8,val:'freeZone'},{type:9,val:'popularCourses'},
    {type:10,val:'consult'},{type:11,val:'special'},{type:12,val:'classify'},{type:13,val:'tag'},{type:14,val:'knowledge'},{type:15,val:'assemble'},{type:16,val:'live'},
]
export default {
    data(){
        return{
    ...
    computed:{
         ...
         */
        homeEditData(){
            return this.$store.state.homeConfig.adultHomeEditData;
        },
        /**
         * 左侧首页当前编辑数据
         */
         ...
         */
        adultHomeConfig(){
            return this.$store.state.homeConfig.adultHomeConfig;
        },
        /**
         * 右侧APP少儿首页模块数据
         */
        ...
        style(){
            ...
            if(cur){
                let obj = this.$store.state.homeConfig.adultHomeModuleChildData.find(item=>item.localId==cur.localId);
    ...
    components:{
        ...
        'columnsCourseEdit':ColumnsCourseEdit,
        'guessLike':GuessLike,
        'guessLikeEdit':GuessLikeEdit,
        'customize':Customize,
        ...
        'popularCoursesEdit':PopularCoursesEdit,
        'consult':Consult,
        'consultEdit':ConsultEdit,
        'special':Special,
    ...
    watch:{
        ...
        homeCloumnId(cur,old){
            if(cur>=0){
                this.cloumnId = cur;
                this.cloumnName = this.homeEditData.find(item=>item.tabId==cur).tabName;
    ...
    beforeRouteLeave (to, from, next){
        ...
        if(this.leaveFlag){
            this.curEdit = ''
            this.editFlag = false;
            this.setAdultHomeRender(false)
            this.setAdultHomeConfig([])
            this.setAdultHomeEditData([])
            this.setAdultHomeModuleChildData([])
            next();
            return;
        }
        let index = this.adultHomeModuleChildData.findIndex(item=>item.inEdit==true);
    ...
    methods:{
        ...mapActions(['setAdultHomeConfig','setAdultHomeEditData','setAdultHomeModuleChildData','setAdultHomeRender','setHomeCloumnId']),
        /**
         * 滚动动画
         * element 元素
         * to 位置
        ...
        getColumnList(){
            ...
            this.$http.get(`${this.$server.getWxColumnList}/3`).then(res=>{
                if(res.status==200){ 
                    let adultHomeEditData = res.content.map((item,index)=>{
                        return {
                            sort:item.sort,
                            tabId:item.id,
                            tabName:item.name,
                    ...
                    if(this.homeEditData.length>0){
                        ...
                        if(obj){
                            let tabId = obj.tabId;
                            let index = adultHomeEditData.findIndex(item=>item.tabId==tabId)
                            if(index>=0){
                                adultHomeEditData.forEach(item=>item.active = false)
                                adultHomeEditData[index].active = true;
                            }
                        }
                    }
                    
                    this.cloumnId = adultHomeEditData.length>0?adultHomeEditData.find(item=>item.active).tabId:'';
                    this.cloumnName = adultHomeEditData.length>0?adultHomeEditData.find(item=>item.active).tabName:'';
                    this.adultHomeEditDataArr = [...adultHomeEditData];
                    this.setAdultHomeEditData(adultHomeEditData)
                    this.$nextTick(()=>{
        ...
        getAppHomePage(loadingInstance){
            ...
            this.$http.get(this.$server.moduleList,{params}).then(res=>{
                if(res.status==200){
                    ...
                    this.setAdultHomeRender(true)
                    if(res.content.length>0){
                        let curHomeEditData = [],adultHomeModuleChildData = [];
                        ...
                        this.setAdultHomeModuleChildData(adultHomeModuleChildData);
                        this.setAdultHomeEditData(this.homeEditData);
                    }
                    this.$nextTick(()=>{
                        if(this.myscroll==''){
        ...
        cloumnSort(obj,flag){
            ...
            this.$http.post(this.$server.columnWxInfoSort,query).then(res=>{
                loadingInstance.close()
                if(res.status==200){
                    if(flag=='prev'){
        ...
        editCloumnName(){
            ...
            if(this.cloumnFlag=='add'){
                ...
                this.curEdit = '';
                this.setAdultHomeEditData(temp)
                this.setAdultHomeModuleChildData([])
                ...
                this.$http.post(this.$server.modWxColumnName,query).then(res=>{
                    loadingInstance.close();
                    if(res.status==200){
         ...
         */
        switchTab(obj){
            if(this.adultHomeModuleChildData.length==0){
                this.$message.error('当前栏目没有模块数据，请添加模块或者删除栏目后再切换！')
                return;
            }
            let index = this.adultHomeModuleChildData.findIndex(item=>item.inEdit==true&&item.subType==this.cloumnId)
            if((index==-1&&this.focusFlag)||this.delLastFlag||index>=0){
                this.$confirm('当前栏目模块没有保存，请先保存！若未保存且切换至其他栏目，当前栏目数据将失效，是否切换？', '切换确认', {
                    ...
                    this.homeEditData.forEach(item=>{
                        item.active = false;
                        // item.data = [];
                    })
                    obj.active = true;
        ...
        addModuleHandle(moduleName,localId){
            ...
            this.adultHomeModuleChildData.push(
        ...
        delColumn(obj,index){
            ...
            this.$confirm('确定要删除这个栏目吗？', '删除确认', {
                ...
                if(obj.tabId==''){
                    ...
                    let i = this.adultHomeModuleChildData.findIndex(item=>item.localId==localId);
                    this.adultHomeModuleChildData.splice(i,1);
                    if(obj.active){
                    ...
                    if(temp.length==1){
                        ...
                        this.$http.post(this.$server.delWxColumnInfo,query).then(res=>{
                            loadingInstance.close()
                            if(res.status==200){
                                this.setAdultHomeRender(false)
                                this.curEdit = '';
        ...
        delModule(){
            this.$confirm('确定要删除这个模块吗？', '删除确认', {
                ...
                let editData = this.adultHomeModuleChildData.find(item=>item.localId==obj.localId)
                let _index = this.adultHomeModuleChildData.findIndex(item=>item.localId==obj.localId)
                ...
                this.adultHomeModuleChildData.splice(_index,1)
                clickFlag = true;
                this.$nextTick(()=>{
                    if(index==0){
        ...
        sortUp(){
            ...
            this.adultHomeModuleChildData.find(item=>item.localId==localId).compareFlag = true
            this.editFlag = true;
            setTimeout(()=>{
                for(let item of this.$refs.myModule){
                    if(item.moduleData.isFocus){
         ...
         */
        saveAppHomeData(){
            let obj = this.adultHomeModuleChildData.find(item=>item.inEdit==true&&item.subType==this.cloumnId)
            if(obj){
