    ...
    <div class="child-home-setting clearfix">
                ...
                <div v-for="(item,index) in homeEditData" :key="index" class="item" :class="item.active?'active':''" @click="switchTab(item)">
                    <el-dropdown size="small" placement="bottom-start">
                        <p class="el-dropdown-link">
                            <i class="el-icon-sort prev" v-if="$authJudge('homeset:appchildren:sort')" v-show="homeEditData.length>1&&index>0" @click.stop="cloumnSort(item,'prev')"></i>
                            <span>{{item.tabName}}</span>
                            <i class="el-icon-sort next" v-if="$authJudge('homeset:appchildren:sort')" v-show="homeEditData.length>1&&index<homeEditData.length-1" @click.stop="cloumnSort(item,'next')"></i>
                        </p>
                        <el-dropdown-menu slot="dropdown" v-if="$authJudge('homeset:appchildren:delcolumn')||$authJudge('homeset:appchildren:editcolumn')">
                            <el-dropdown-item v-if="$authJudge('homeset:appchildren:delcolumn')" @click.native="delColumn(item,index)">删除</el-dropdown-item>
                            <el-dropdown-item v-if="$authJudge('homeset:appchildren:editcolumn')" @click.native="openCloumnNameDialog(item)">修改</el-dropdown-item>
                        </el-dropdown-menu>  
                    </el-dropdown>
                </div>
            </div>
            <div class="add" v-if="$authJudge('homeset:appchildren:addcolumn')">
        ...
        <div class="left-box" :class="focusFlag?'in-edit':''">
            ...
            <div class="module-edit-btns">
                <p v-if="$authJudge('homeset:appchildren:upmodule')" @click="sortUp" :class="moveUp?'allow-move-up':''"><i class="el-icon-caret-top"></i><span>上移</span></p>
                <p v-if="$authJudge('homeset:appchildren:downmodule')" @click="sortBottom" :class="moveDown?'allow-move-down':''"><i class="el-icon-caret-bottom"></i><span>下移</span></p>
                <p v-if="$authJudge('homeset:appchildren:delmodule')" @click="delModule"><i class="el-icon-delete"></i><span>删除</span></p>
            </div>
            <div class="btns" v-if="curHomeEditData.length>0||delLastFlag">
                <div v-if="focusFlag||delLastFlag">
                    <el-button v-if="$authJudge('homeset:appchildren:savemodule')" size="medium" type="primary" @click="saveAppHomeData">保存</el-button>
        ...
        <div class="right-box handle">
            <transition mode="out-in">
                <component :is="curEdit" v-if="curEdit!=''" :settingType="'2'"></component>
            </transition>
        </div>
...
import ClassifyNav from './components/childComponents/classifyNavApp'//分类\标签
import ClassifyNavEdit from './components/childComponents/classifyNavEditApp'//分类\标签编辑
...
import GiveLesson from './components/childComponents/giveLesson'//赠课活动
import GiveLessonEdit from './components/childComponents/giveLessonEdit'//赠课活动编辑
...
const subModule = [
    ...
    {type:16,val:'seriesCourse'},{type:17,val:'goods'},{type:18,val:'giveLesson'}
]
export default {
    data(){
        return{
    ...
    methods:{
        ...
        getColumnList(){
            ...
            this.$http.get(`${this.$server.getColumnList}/2`).then(res=>{
                if(res.status==200){ 
                    ...
                    this.cloumnId = childHomeEditData.find(item=>item.active).tabId;
                    this.cloumnName = childHomeEditData.find(item=>item.active).tabName;
