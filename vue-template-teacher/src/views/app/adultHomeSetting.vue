    ...
    <div class="adult-home-setting clearfix">
                ...
                <div v-for="(item,index) in homeEditData" :key="index" class="item" :class="item.active?'active':''" @click="switchTab(item)">
                    <el-dropdown size="small" placement="bottom-start">
                        <p class="el-dropdown-link">
                            <i class="el-icon-sort prev" v-if="$authJudge('homeset:appadult:sort')" v-show="homeEditData.length>1&&index>0" @click.stop="cloumnSort(item,'prev')"></i>
                            <span>{{item.tabName}}</span>
                            <i class="el-icon-sort next" v-if="$authJudge('homeset:appadult:sort')" v-show="homeEditData.length>1&&index<homeEditData.length-1" @click.stop="cloumnSort(item,'next')"></i>
                        </p>
        ...
        <div class="left-box" :class="focusFlag?'in-edit':''">
            ...
            <div class="module-edit-btns">
                <p v-if="$authJudge('homeset:appadult:upmodule')" @click="sortUp" :class="moveUp?'allow-move-up':''"><i class="el-icon-caret-top"></i><span>上移</span></p>
                <p v-if="$authJudge('homeset:appadult:downmodule')" @click="sortBottom" :class="moveDown?'allow-move-down':''"><i class="el-icon-caret-bottom"></i><span>下移</span></p>
                <p v-if="$authJudge('homeset:appadult:delmodule')" @click="delModule"><i class="el-icon-delete"></i><span>删除</span></p>
            </div>
            <div class="btns" v-if="curHomeEditData.length>0||delLastFlag">
                <div v-if="focusFlag||delLastFlag">
                    <el-button v-if="$authJudge('homeset:appadult:savemodule')" size="medium" type="primary" @click="saveAppHomeData">保存</el-button>
...
import ClassifyNav from './components/adultComponents/classifyNavApp'//分类\标签展示
import ClassifyNavEdit from './components/adultComponents/classifyNavEditApp'//分类\标签编辑
...
import Ranking from './components/adultComponents/ranking'
import RankingEdit from './components/adultComponents/rankingEdit'
import Goods from './components/adultComponents/goods'//商品
import GoodsEdit from './components/adultComponents/goodsEdit'//商品编辑
import GiveLesson from './components/adultComponents/giveLesson'//赠课活动
import GiveLessonEdit from './components/adultComponents/giveLessonEdit'//赠课活动编辑
...
const subModule = [
    ...
    {type:17,val:'ranking'},{type:18,val:'goods'},{type:19,val:'giveLesson'}
]
export default {
    data(){
        return{
    ...
    methods:{
        ...
        getColumnList(){
            ...
            this.$http.get(`${this.$server.getColumnList}/1`).then(res=>{
                if(res.status==200){ 
                    ...
                    this.cloumnId = adultHomeEditData.find(item=>item.active).tabId;
                    this.cloumnName = adultHomeEditData.find(item=>item.active).tabName;
        ...
        getAppHomePage(loadingInstance){
            ...
            this.$http.get(this.$server.getAppHomePage,{params}).then(res=>{
                if(res.status==200){
                    if(loadingInstance)
        ...
        addModuleHandle(moduleName,localId){
            ...
            console.log('this.adultHomeModuleChildData',this.adultHomeModuleChildData);
        ...
        saveAppHomeData(){
            ...
            this.$http.post(this.$server.saveSubModuleSort,query).then(res=>{
                if(res.status==200){
                    this.setAdultHomeRender(false)
                    this.curEdit = '';
                    this.editFlag = false;
