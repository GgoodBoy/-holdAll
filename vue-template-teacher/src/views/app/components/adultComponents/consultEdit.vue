<template>
    <div class="module-edit consult">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">资讯</p>
        ...
        <div class="module-edit-body">
            ...
            <div class="number">
                ...
                <el-tooltip class="item" effect="dark" :content="styleVal==1?'该样式最多可以添加5个':'该样式最多可以添加3个'" placement="right">
                    <i class="question el-icon-question"></i>
                </el-tooltip>
            </div>
            <div class="select-course">
                <p>资讯</p>
                <el-button type="primary" size="medium" @click="showDialog">选择资讯</el-button>
            </div>
            <div class="edit-list">
        ...
        <el-dialog :custom-class="'edit-dialog'" :visible.sync="editDialogVisible" :width="'500px'" :show-close="false">
            <div class="search-box">
                <el-input placeholder="输入资讯名称查询" size="medium" clearable prefix-icon="el-icon-search" v-model="searchKey" maxlength="10" @keypress.native="pressEvent($event)"></el-input>
...
export default {
    ...
    watch:{
        styleVal(cur,old){
            ...
            this.EditDialogMaxNum = cur=='1'?5:3;
        },
    ...
    methods:{
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',consultData:[],active:true},
                        {styleType:'2',consultData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${this.$server.getSubModuleInfoList}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let consultData = res.content.ahprrList.map((item,index)=>{
                        return {
                    ...
                    let editChildData = [
                        {styleType:'1',consultData:res.content.style==1?consultData:[],active:res.content.style==1?true:false},
                        {styleType:'2',consultData:res.content.style==2?consultData:[],active:res.content.style==2?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        saveData(){
            ...
            this.curEditData.forEach((item,index)=>{
                ...
                query['appHomeRelationRecordFormList['+index+'].type'] = 2;
            ...
            this.$http.post(this.$server.saveSubModule,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).consultData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
