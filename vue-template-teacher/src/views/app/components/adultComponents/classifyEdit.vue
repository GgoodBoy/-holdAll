<template>
    <div class="module-edit classify-edit">
        ...
        <div class="module-edit-body">
            ...
            <div class="add-btn"><button class="my-primary-btn" @click="addClassify" :class="{'not-allow':curEditData.length>=3}"><i class="el-icon-plus"></i><span>添加课程</span></button></div>
        </div>
...
export default {
    ...
    methods:{
        ...
        getSubModuleInfoList(id,data){
            ...
            this.$http.get(`${url}/${id}`).then(res=>{
                if(res.status==200){
                    ...
                    let classifyData = res.content.ahprrList.map((item,index)=>{
                        return {
                            imgUrl:item.imgUrl,
                            title:item.title,
                    ...
                    let editChildData = data.map(item=>{
                        return {
                            targetId:item.id,
                            classifyData:item.id==this.targetId?classifyData:[],
         ...
         */
        getClassifyList(){
            this.$http.get(`${this.$server.getClassifyList}/1`).then(res=>{
                if(res.status==200){
