<template>
    <div class="module-edit customize">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">自定义</p>
        ...
        <div class="module-edit-body">
            ...
            <div class="number">
                ...
                <el-tooltip class="item" effect="dark" :content="styleVal=='1'?'该样式最多可以添加20个':'该样式最多可以添加6个'" placement="right">
                    <i class="question el-icon-question"></i>
                </el-tooltip>
            </div>
            <p class="tip" v-if="styleVal=='2'">尺寸：432*400<span>支持JPEG、PNG</span></p>
            <div class="select-course" v-if="styleVal=='1'">
                <p>课程</p>
                <el-button type="primary" size="medium" @click="showDialog">选择课程</el-button>
            </div>
            <div class="edit-list">
                <div class="edit-item clearfix" :class="styleVal=='1'?'type2':''" v-for="(item,index) in curEditData" :key="index">
                    <div class="img-file-box" @click="uploadFile(index)" v-if="styleVal=='2'">
                    ...
                    <div class="text-box" v-if="styleVal=='2'">
                    ...
                    <p class="course-name" v-if="styleVal=='1'">{{item.courseName}}</p>
                    <div class="btns">
                        <div class="del-btn" @click="delItem(index)">
                            <i class="el-icon-circle-close"></i>
                        </div>
                        <div class="sort-btn" :class="styleVal=='1'?'sort-type2':''">
            ...
            <div class="add-btn" v-if="styleVal=='2'"><button class="my-primary-btn" @click="addcolumnCourse" :class="{'not-allow':curEditData.length>=6}"><i class="el-icon-plus"></i><span>添加课程</span></button></div>
        </div>
        <el-dialog :custom-class="'edit-dialog'" :visible.sync="editDialogVisible" :width="'500px'" :show-close="false">
            ...
            <div class="list" ref="scrollBox">
                <el-radio-group v-model="editDialogVal" v-if="styleVal=='2'">
...
export default {
    ...
    computed:{
        ...
        curEditData(){
            if(this.editData.data&&this.editData.data.length>0){
                ...
                return temp?temp.customizeData:[];
            }
            else{
                return [];
            }
    ...
    methods:{
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',customizeData:[],active:true},
                        {styleType:'2',customizeData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${url}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let customizeData = [];
                    if(res.content.style==2){
                        customizeData = res.content.ahprrList.map((item,index)=>{
                            return {
                                imgUrl:item.imgUrl,
                                title:item.title,
                        ...
                        this.dialogArr = res.content.ahprrList.map(item=>{
                    ...
                    let editChildData = [
                        {styleType:'1',customizeData:res.content.style==1?customizeData:[],active:res.content.style==1?true:false},
                        {styleType:'2',customizeData:res.content.style==2?customizeData:[],active:res.content.style==2?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        addcolumnCourse(){
            if(this.styleVal=='2'){
                if(this.curEditData.length>=6) return;
                this.curEditData.push({
                    imgUrl:'',
                    title:'',
                    courseName:'',
        ...
        saveData(){
            ...
            this.$http.post(url,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).customizeData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
            ...
            .styles{
                ...
                .real-img{
                    ...
                    &.style1{
                        ...
                        background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/chengren/mk1/zdy/zdy1.png')no-repeat 50% 50% /cover;
                    }
                    &.style2{
                        width:375px;
                        height: 100px;
                        transform: scale(0.8) translate(-10%,-10%);
                        background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/chengren/mk1/zdy/zdy2.png')no-repeat 50% 50% /cover;
                    }
                }
            }
            .number{
