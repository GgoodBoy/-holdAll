    ...
    <div class="module-edit classify-edit">
        ...
        <div class="module-edit-body">
            ...
            <div class="number">
                ...
                <el-tooltip class="item" effect="dark" content="最多可以添加9条数据" placement="right">
...
export default {
    ...
    computed:{
        ...
        curEditData(){
            if(this.editData.data&&this.editData.data.length>0){
                ...
                let obj = temp.tagData.find(item=>item.active==true)
                return obj?obj.data:[]
            }
            else{
                return [];
    ...
    watch:{
        targetId(cur,old){
            let temp = this.editData.data.find(item=>item.active==true);
            temp.targetId = cur;
            let obj = temp.tagData.find(item=>item.targetId==cur);
            temp.tagData.forEach(item=>item.active=false)
            if(obj)
                obj.active = true;
        },
        styleVal(cur,old){
    ...
    methods:{
        ...
        getSubModuleInfoList(id,data){
            this.$http.get(`${this.$server.getSubModuleInfoList}/${id}`).then(res=>{
                if(res.status==200){
                    ...
                    let editChildData = [
                        {styleType:'1',tagData:temp1,active:style==1?true:false,targetId:style==1?res.content.targetId:''},
                        {styleType:'2',tagData:temp2,active:style==2?true:false,targetId:style==2?res.content.targetId:''},
                        {styleType:'3',tagData:temp3,active:style==3?true:false,targetId:style==3?res.content.targetId:''}
                    ]
                ...
                if(res.status==200){
                    ...
                    if(this.editData.data.length==0){
                        if(id==-1){
                            ...
                            let editChildData = [
                                {styleType:'1',tagData:temp1,active:true,targetId:''},
                                {styleType:'2',tagData:temp2,active:false,targetId:''},
                                {styleType:'3',tagData:temp3,active:false,targetId:''}
                            ]
                            this.editData.data = editChildData;
                        }else{
        ...
        saveData(){
            ...
            this.$http.post(this.$server.saveSubModule,query).then(res=>{
            ...
            .styles{
                ...
                .real-img{
                    ...
                    &.style1{
                        ...
                        background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/youer/mk2/bq/bq1.png')no-repeat 50% 50% /cover;
                    ...
                    &.style2{
                        ...
                        border-radius: 5px;
                        transform: scale(0.8) translate(-10%,-10%);
                        background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/youer/mk2/bq/bq2.png')no-repeat 50% 50% /cover;
                    }
                }
            }
            .number{
