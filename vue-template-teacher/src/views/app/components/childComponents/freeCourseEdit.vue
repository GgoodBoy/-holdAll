<template>
    <div class="module-edit new-online">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">免费课程</p>
    ...
    computed:{
        ...
        curEditData(){
            if(this.editData.data&&this.editData.data.length>0){
                ...
                return temp?temp.freeCourseData:[];
            }
            else{
                return [];
            }
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',freeCourseData:[],active:true},
                        {styleType:'2',freeCourseData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${this.$server.getSubModuleInfoList}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let freeCourseData = res.content.ahprrList.map((item,index)=>{
                        return {
                    ...
                    let editChildData = [
                        {styleType:'1',freeCourseData:res.content.style==1?freeCourseData:[],active:res.content.style==1?true:false},
                        {styleType:'2',freeCourseData:res.content.style==2?freeCourseData:[],active:res.content.style==2?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        saveData(){
            ...
            this.$http.post(this.$server.saveSubModule,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).freeCourseData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
