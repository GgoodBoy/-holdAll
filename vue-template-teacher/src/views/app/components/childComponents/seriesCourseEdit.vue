<template>
    <div class="module-edit seriesCourse">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">系列课程</p>
        ...
        <div class="module-edit-body">
            <p class="tip">尺寸:300*148    支持PNG、JPEG，只能添加2个或者4个系列</p>
            <div class="edit-list">
                <div class="edit-item clearfix" v-for="(item,index) in classifyEditData.data" :key="index">
                    ...
                    <div class="select-area">
                        <div class="type">
                            <label>分类类型</label>
                            <el-select v-model="item.type" size="small" placeholder="选择类型">
                                <el-option
            ...
            <div class="add-btn"><button class="my-primary-btn" @click="addClassify"><i class="el-icon-plus"></i><span>添加系列课程</span></button></div>
        </div>
     </div>    
</template>
<script>
...
export default {
    ...
    methods:{
                    ...
                    res.content.ahprrList.forEach(item=>{
                        this.classifyEditData.data.push({
                            ...
                            arr:[
                                {
                                    type:String(item.type),
                                    value:Number(item.targetId)
                                }
                            ],
                            curList:item.type==6?[...this.labelList]:[...this.classifyList]
                        })
                    })
        ...
        addClassify(){
            this.classifyEditData.data.push({
                targetId:'',type:'7',img:'',arr:[],curList:[]
            })
        },
        /**
         * 点击上传
        ...
        getFile(event,index){
            ...
            reader.onload = e=>{
                ...
                image.onload = ()=>{
                    ...
                    if(width!=300||height!=148){
                        this.$message.error('请上传300*148的图片');
                        return;
                    }
        ...
        saveData(){
            ...
            if(this.classifyEditData.data.length%2!=0){
                this.$message.error('只能添加2个或者4个系列课程')
                return;
            }
            let flag = this.classifyEditData.data.some(item=>{
                return item.img == '' || item.targetId == ''
            })
            if(flag){
                this.$alert('您还有未添加的数据，不能保存', '提示', {
                    confirmButtonText: '关闭',
            ...
            for(let i in arr){
                ...
                let obj = tempArr.find(item=>(item.targetId==arr[i].targetId))
                if(obj){
                    repeatFlag = true;
                    break;
                }
