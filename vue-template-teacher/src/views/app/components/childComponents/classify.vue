<template>
    <div class="classify-module module">
        <p class="module-name clearfix">
            <span class="name">{{curObj?curObj.name:''}}</span>
            <span class="more">更多>></span>
        </p>
        <div :class="moduleData.isFocus?'is-focus':''">
            <div class="occupancy-map" :class="`style${curObj.style}`"></div>
        </div>
    </div>
</template>
<script>
export default {
    name:'classify',
    props:['moduleData'],
    data(){
        return{
            curObj:''
        }
    },
    created(){
        this.curObj = this.$store.state.homeConfig.childHomeModuleChildData.find(item=>item.localId==this.moduleData.localId);
    },
    computed:{
        curHomeEditData(){
            return this.$store.state.homeConfig.childHomeEditData.find(item=>item.active==true).data
        },
    },
    watch:{
        curHomeEditData(cur,old){
            this.curObj = this.$store.state.homeConfig.childHomeModuleChildData.find(item=>item.localId==this.moduleData.localId)||{};
        }
    }
}
</script>
<style lang="scss" scoped>
    .classify-module{
        .is-focus{
            padding: 4px;
            border:1px solid #e32513;
            font-size: 0;
        }
        .module-name{
            line-height: 16px;
            margin-bottom:10px;
            padding: 0 15px;
            .name{
                float: left;
                font-size: 16px;
                font-weight: bold;
            }
            .more{
                float: right;
                font-size: 14px;
            }
        }
        .occupancy-map{
            width: 345px;
            margin:0 auto;
            &.style1{
                height: 140px;
                background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/youer/mk2/fl/ys1.png')no-repeat 50% 50% /cover;
            }
            &.style2{
                height: 158px;
                background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/youer/mk2/fl/ys2.png')no-repeat 50% 50% /cover;
            }
            &.style3{
                height: 158px;
                background: url('../../../../assets/images/ys3.png')no-repeat 50% 50% /cover;
            }
        }
    }
</style>
