<template>
    <div class="module-edit free-zone">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">免费专区</p>
        ...
        <div class="module-edit-body">
            ...
            <div class="clearfix styles">
                ...
                <div class="question">
                    ...
                    <div class="real-img" :class="{'style1':styleVal==1,'style2':styleVal==2,'style3':styleVal==3}"></div>
                </div>
            </div>
            <div class="number">
                <p>展示数量</p>
...
export default {
    ...
    computed:{
        ...
        editData(){
            ...
            if(obj){
                ...
                let temp = this.childHomeModuleChildData.find(item=>item.localId==localId);
                return temp;
            }else{
                return []
            }
    ...
    watch:{
        styleVal(cur,old){
            ...
            this.EditDialogMaxNum = cur==1?6:12;
        },
        ...
        dialogArrStyle3(cur,old){
            ...
            cur.forEach((item)=>{
                let index = this.courseCheckListStyle3.findIndex(option=>option.id==item);
                if(index==-1){
                    let obj = this.editDialogList.find(option=>option.id==item);
                    this.courseCheckListStyle3.push({
                        id:obj.id,
                        title:obj.title
                    })
                }
    ...
    methods:{
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',freeZoneData:[],active:true},
                        {styleType:'2',freeZoneData:[],active:false},
                        {styleType:'3',freeZoneData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${this.$server.getSubModuleInfoList}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let freeZoneData = res.content.ahprrList.map((item,index)=>{
                        return {
                    ...
                    }else if(res.content.style==3){
                        this.courseCheckListStyle3 = res.content.ahprrList.map(item=>{
                            return {
                                id:Number(item.targetId),
                                title:item.title
                            }
                    ...
                    let editChildData = [
                        {styleType:'1',freeZoneData:res.content.style==1?freeZoneData:[],active:res.content.style==1?true:false},
                        {styleType:'2',freeZoneData:res.content.style==2?freeZoneData:[],active:res.content.style==2?true:false},
                        {styleType:'3',freeZoneData:res.content.style==3?freeZoneData:[],active:res.content.style==3?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        showDialog(){
            ...
            }else if(this.styleVal=='3'){
                this.dialogArrStyle3.splice(0,this.dialogArrStyle3.length)
                this.curEditData.forEach(item=>{
                    if(item.targetId){
                        this.dialogArrStyle3.push(item.targetId)
                    }
                })
            }
            this.getFreeListForPeriod();
            ...
            this.$http.get(this.$server.getFreeListForPeriod,{params}).then(res=>{
                if(res.status==200){
                    ...
                    if(this.styleVal=='1'){
                        ...
                        if(this.dialogArrStyle3.length>=this.EditDialogMaxNum){
                            arr.forEach(item=>{
                                item.disabled = true;
                            })
                        }else{
                            arr.forEach(item=>{
                                let obj = this.dialogArrStyle3.find(id=>id==item.id);
                                if(obj){
                                    item.disabled = true;
                                    item.oldData = true;
                                }else{
        ...
        dialogSure(){
            ...
            if(this.styleVal=='1'){
                ...
                this.courseCheckListStyle3.forEach(item=>{
                    let obj = this.dialogArrStyle3.find(id=>id==item.id);
                    if(obj){
                        this.curEditData.push({
                            targetId:item.id,
        ...
        delItem(index){
            ...
            if(this.styleVal=='1'){
                ...
                let i = this.dialogArrStyle3.findIndex(id=>id==targetId)
                this.dialogArrStyle3.splice(i,1)
            }
        },
        /**
         * 元素上移
        ...
        saveData(){
            ...
            this.$http.post(this.$server.saveSubModule,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).freeZoneData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
            ...
            .styles{
                ...
                .real-img{
                    ...
                    &.style1{
                        ...
                        background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/youer/mk1/zxsx/zxsx2.png')no-repeat 50% 50% /cover;
                    }
                    &.style2{
                        width: 345px;
                        height: 187px;
                        border-radius: 5px;
                        transform: scale(0.8) translate(-10%,-10%);
                        background: url('https://xgxw-pic.oss-cn-beijing.aliyuncs.com/zwt/youer/mk1/zxsx/zxsx1.png')no-repeat 50% 50% /cover;
                    }
                    &.style3{
                         width: 345px;
                         height: 160px;
                         border-radius: 5px;
                         transform: scale(0.8) translate(-10%,-10%);
                         background: url('../../../../assets/images/4.png')no-repeat 50% 50% /cover;
                     }
                }
            }
            .number{
