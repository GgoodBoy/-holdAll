<template>
    <div class="module-edit new-online">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">最新上线</p>
        ...
        <div class="module-edit-body">
            ...
            <div class="number">
                ...
                <el-tooltip class="item" effect="dark" content="该样式最多可以添加6个" placement="right">
    ...
    computed:{
        ...
        curEditData(){
            if(this.editData.data&&this.editData.data.length>0){
                ...
                return temp?temp.newOnlineData:[];
            }
            else{
                return [];
            }
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',newOnlineData:[],active:true},
                        {styleType:'2',newOnlineData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${this.$server.getSubModuleInfoList}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let newOnlineData = res.content.ahprrList.map((item,index)=>{
                        return {
                    ...
                    let editChildData = [
                        {styleType:'1',newOnlineData:res.content.style==1?newOnlineData:[],active:res.content.style==1?true:false},
                        {styleType:'2',newOnlineData:res.content.style==2?newOnlineData:[],active:res.content.style==2?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        saveData(){
            ...
            this.$http.post(this.$server.saveSubModule,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).newOnlineData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
