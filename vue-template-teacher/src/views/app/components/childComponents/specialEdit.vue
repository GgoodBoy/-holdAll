<template>
    <div class="module-edit special">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">专题</p>
        ...
        <div class="module-edit-body">
            ...
            <div class="select-course">
                <p>专题</p>
                <el-button type="primary" size="medium" @click="showDialog">选择专题</el-button>
            </div>
            <div class="edit-list">
        ...
        <el-dialog :custom-class="'edit-dialog'" :visible.sync="editDialogVisible" :width="'500px'" :show-close="false">
            <div class="search-box">
                <el-input placeholder="输入专题名称查询" size="medium" clearable prefix-icon="el-icon-search" v-model="searchKey" maxlength="10" @keypress.native="pressEvent($event)"></el-input>
...
export default {
    ...
    methods:{
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',specialData:[],active:true},
                        {styleType:'2',specialData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${this.$server.getSubModuleInfoList}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let specialData = res.content.ahprrList.map((item,index)=>{
                        return {
                    ...
                    let editChildData = [
                        {styleType:'1',specialData:res.content.style==1?specialData:[],active:res.content.style==1?true:false},
                        {styleType:'2',specialData:res.content.style==2?specialData:[],active:res.content.style==2?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        showDialog(){
            ...
            this.getThematicActivityList();    
        },
        /**
         * 获取专题
         */
        getThematicActivityList(item){
            this.loading = true;
            ...
            this.$http.get(this.$server.getThematicActivityList,{params}).then(res=>{
                if(res.status==200){
        ...
        saveData(){
            ...
            this.curEditData.forEach((item,index)=>{
                ...
                query['appHomeRelationRecordFormList['+index+'].type'] = 5
            ...
            this.$http.post(this.$server.saveSubModule,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).specialData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
