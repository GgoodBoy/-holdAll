<template>
    <div class="module-edit free-zone">
        <div class="module-edit-header clearfix">
            <p class="title" ref="title">名师推荐</p>
        ...
        <div class="module-edit-body">
            ...
            <div class="number">
                ...
                <el-tooltip class="item" effect="dark" :content="settingType=='4'?'该样式最多可以添加10个':'该样式最多可以添加12个'" placement="right">
...
export default {
    ...
    created(){
        setTimeout(()=>{
            ...
            this.EditDialogMaxNum = this.settingType=='4'?10:12;
        },1)
    },
    mounted(){
        this.initData();
    ...
    methods:{
        ...
        initData(){
            ...
            if(this.editData.data.length==0){
                if(id==-1){
                    this.styleVal = '1';
                    let editChildData = [
                        {styleType:'1',commendData:[],active:true},
                        {styleType:'2',commendData:[],active:false}
                    ]
        ...
        getSubModuleInfoList(id){
            ...
            this.$http.get(`${url}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    let commendData = res.content.ahprrList.map((item,index)=>{
                        return {
                    ...
                    let editChildData = [
                        {styleType:'1',commendData:res.content.style==1?commendData:[],active:res.content.style==1?true:false},
                        {styleType:'2',commendData:res.content.style==2?commendData:[],active:res.content.style==2?true:false}
                    ]
                    this.editData.data = editChildData;
        ...
        saveData(){
            ...
            this.curEditData.forEach((item,index)=>{
                ...
                query['appHomeRelationRecordFormList['+index+'].type'] = 9
            ...
            this.$http.post(url,query,{headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.editData.data.find(item=>item.active == false).commendData = []
                    this.editData.inEdit = false;
                    this.editData.saveDataFlag = true;
                    this.$message({
                        message: '保存成功！',
