    ...
    methods:{
            ...
            restructureData(){
                            ...
                            selectlist.push(
                                {
                                    ...
                                    'coursePrice':iterator.sellingPrice||0,
                                    "reducePrice":0,
                                    "discount":0,
                                    "PromotionPrice":0
                                }
                ...
                if(idLocalStorArr2.length>0){
                    let localCourseData = JSON.parse(sessionStorage.getItem('timeLimitCourseListVal'));
                    if(localCourseData){
                        idLocalStorArr2.forEach((element,index)=>{
                            for (const iterator of localCourseData) {
                                if(Number(iterator.courseId)==element){
                                    localArr.push(
                                        {
                                            "id":iterator.id,
                                            "courseId":iterator.courseId,
                                            "courseName":iterator.courseName,
                                            'coursePrice':iterator.coursePrice||0,
                                            "reducePrice":iterator.reducePrice,
                                            "discount":iterator.discount,
                                            "PromotionPrice":iterator.PromotionPrice
                                        }
                                    );
                                    break;
                                }
                ...
                sessionStorage.removeItem('timeLimitCourseListVal'); 
                selectlist = localArr.concat(selectlist);
