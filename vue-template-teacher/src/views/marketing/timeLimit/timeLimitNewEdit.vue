    ...
    <div class="timeLimitNewEdit">
        ...
        <div class="editContentTop"> 
            <div class="unifiedCss">
                <div class="titles beforeCss" :class="{ 'titlesTop1': !ishowArr.title ,'titlesTop2': ishowArr.title}">活动名称</div>
                <div class="indexCss">
                    <el-input style="width:250px" maxlength='20' v-model="title" placeholder="请输入内容" show-word-limit></el-input>
                    <div v-if="ishowArr.title" style="color:#b4272d;">活动名称不能为空</div>
                </div>
            </div>
            <div class="unifiedCss">
                <div class="titles beforeCss" :class="{ 'titlesTop3': !timeBool ,'titlesTop4': timeBool}">活动时间</div>
                <div>
                    <el-date-picker
                        v-model="beginAt"
                        type="datetime"
                    ...
                    <div v-if="ishowArr.beginAt" style="color:#b4272d;">请选择开始时间</div>
                </div>
                ...
                <div style="margin-left:10px;">
                    ...
                    <div v-if="ishowArr.endAt" style="color:#b4272d;">请选择结束时间</div>
                </div>                
            </div>                          
            <div class="unifiedCss" style="height:40px;">
                <div class="titles beforeCss" :class="{ 'titlesTop1': !ishowArr.title ,'titlesTop2': ishowArr.title}">是否使用优惠券</div>
                <div>
                    <el-radio-group v-model="radioCoupons">
        ...
        <div class="editContentBut" >
            <div class="editFilterBox">
                ...
                <div  class="rightBox">
                    <el-button :disabled='loading' style="background:#b4272d;color:#fff;margin-bottom:10px;" @click="selectCourse">添加课程</el-button>
                    ...
                    min-width='400'>
                        <template slot-scope="scope">
                            <div class="tablePromotion">
                                ...
                                <div style="margin-right:10px;">
                                    减价<el-input style="width:100px;margin:0 5px;" @change="inputchange(scope.row,2)" v-model="scope.row.reducePrice" placeholder="0.00"></el-input>元
                                </div>
                                <div>
            ...
            <!-- <div v-else style="text-align: center;margin: 20px 0;">暂时没有加入课程~</div> -->
...
export default {
    ...
    methods:{
            ...
            this.tableDataIdList.forEach((element,index)=>{
                dataList.forEach((elementj,indexj)=>{
                    if(element.courseId==elementj.courseId){
                        ...
                        elementj.PromotionPrice=this.proving(JSON.stringify(Number(elementj.coursePrice)*100*(Number(this.batchDiscountVal)*10)/10000),2);
        ...
        seaveTimeLimitDate(){
            ...
            if(!this.title){
                this.ishowArr.title = true;
                return;
            }
            if(!this.beginAt){
                this.ishowArr.beginAt = true;
                this.timeBool = true;
                return;
            }
            if(!this.endAt){
                this.ishowArr.endAt = true;
                this.timeBool = true;
                return;
            }
            let parems = {
            ...
            self.$http.post(urls,parems).then(res=>{
                if(res.status==200){
                    this.$message.success(Number(this.timeLimitId)>0?'编辑成功':'创建成功');
                ...
                if(type==1){
                    this.tableData.forEach((element,index)=>{
                        ...
                        if(element.sortId==data.sortId){
                            let discountVar = this.proving(element.discount,1);
                            if(!Number(discountVar)){
                                discountVar = 0.1;
                            }
                            element.discount = discountVar;
                            // element.PromotionPrice=this.proving(JSON.stringify(Number(data.coursePrice)/10*Number(data.discount)),2);
                            element.PromotionPrice=this.proving(JSON.stringify(Number(data.coursePrice)*100*(Number(data.discount)*10)/10000),2);
                            element.reducePrice=this.proving(JSON.stringify((Number(data.coursePrice)*100-element.PromotionPrice*100)/100),2);
                            uupdates = element;
                        }
                    });
                    ...
                    this.tableData.forEach((element,index)=>{
                        if(element.sortId==data.sortId){
                            let reducePriceVal = this.proving(element.reducePrice,2);
                            if(Number(reducePriceVal)>Number(data.coursePrice)){
                                reducePriceVal = (Number(data.coursePrice)*100-1)/100;
                            }
                            element.reducePrice = reducePriceVal;
                    ...
                    this.tableData.forEach((element,index)=>{
                        if(element.sortId==data.sortId){
                            let PromotionPriceVal = this.proving(element.PromotionPrice,2);
                            if(Number(PromotionPriceVal)>Number(data.coursePrice)){
                                PromotionPriceVal = (Number(data.coursePrice)*100-1)/100;
                            }
                            element.PromotionPrice = Number(PromotionPriceVal);
                            let reducePriceStr =this.proving(JSON.stringify((Number(data.coursePrice)*100-Number(element.PromotionPrice)*100)/100),2);
                            let lastStr = reducePriceStr.substring(reducePriceStr.length-1)
                            if(reducePriceStr.indexOf('.') != -1){
                                if(lastStr=='0'){
                                    element.reducePrice = reducePriceStr.substring(0,reducePriceStr.length-1);
                                }else{
                                    element.reducePrice = reducePriceStr;
                                }
                            }else{
                                element.reducePrice = reducePriceStr;
                            }
                            
                            let discountVar = this.proving(JSON.stringify(Number(data.PromotionPrice)/Number(data.coursePrice)*10),1);
                            element.discount=discountVar
                            uupdates = element;
                        }
                    }); 
            ...
            if(data.bool){
                ...
                data.datas.forEach((element,index)=>{
                    listCourse.push({
                        ...
                        discount:element.discount||'10.0',//折扣
                ...
                sessionStorage.setItem('timeLimitCourseListVal',JSON.stringify(listCourse));
        ...
        detailDialog(type){
            ...
            if(type==1){
                ...
                self.tableData.forEach((element,index)=>{
                    let i = false;
                    this.tableDataIdList.forEach((elementj,indexj)=>{
                        if(element.courseId==elementj.courseId){
                            i=true;
                        }
                    });
        ...
        getTimeLinitList(){
            ...
            self.$http.get(self.$server.timeLimitDetail+self.timeLimitId).then(res=>{
                if(res.status==200){
                    ...
                    this.beginAt = new Date(contentdate.beginAt);
                    this.radioCoupons = contentdate.useCoupon;
                    this.endAt = new Date(contentdate.endAt);
