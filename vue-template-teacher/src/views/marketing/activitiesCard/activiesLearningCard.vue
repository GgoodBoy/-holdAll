        ...
        <el-row class="lLine40 headerDiv">
            ...
            <el-col class="lRight-r lMarginTop30">
                <el-row>
                    ...
                    <el-col v-if="$authJudge('market:activitystudycard:select')" :span="1"  :offset="1"><el-button class="redBackColor butColorF" @click="findInfo">查询</el-button></el-col>
                    <el-col :span="13" style="text-align: right;">
                        <el-button v-if="$authJudge('market:activitystudycard:add')" class="redBackColor butColorF" @click="creatlearnCard">添加活动学习卡</el-button>
                    </el-col>
                </el-row>
            </el-col>
        </el-row>  
                ...
                style="width: 100%">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <el-row>                                
                                <el-col :span="12">
                                    <el-button type="text" v-if="($authJudge('market:activitystudycard:void')&&scope.row.status==1)||scope.row.status!=1" :disabled="scope.row.status==1?false:true" v-bind:class="{ 'black-btn-ok': scope.row.status==1?true:false, 'black-btn-no': scope.row.status==1?false:true }" class="black-btn" @click="cancelBatch(scope.row,1)">
                                        <span v-if="scope.row.status==1">作废本批次</span>
                                        <span v-else>本批次已作废</span>
                                    </el-button>                                    
                                </el-col>
                                <el-col :span="12">
                                    <el-button v-if="$authJudge('market:activitystudycard:export')" type="text" class="black-btn" @click="exportTable(scope.row)">导出</el-button>
                                </el-col>
                                <!-- <el-col :span="8">  
                                ...
                                <el-col :span="12">
                                    <el-button v-if="$authJudge('market:activitystudycard:del')" type="text" class="black-btn" @click="removeBatch(scope.row,2)">删除</el-button>
                                </el-col>
                                <el-col :span="12">
                                    <el-button v-if="$authJudge('market:activitystudycard:find')" type="text" class="black-btn" @click="inquiryBatch(scope.row,1)">查看</el-button>
                                </el-col>
                                <!-- <el-col :span="8">
...
export default {
    ...
    methods:{
        ...
        // getChannelList(){
        //     var self = this;
        //     self.$http.get(self.$server.getOriginaAtivityStudyCard).then(res=>{
        //         if(res.status==200){    
        ...
        //         cardBaseId:id        
        //     };
        //     self.$http.get(self.$server.getCardCountActivityStudyCard(params)).then(res=>{
        //         if(res.status==200){    
    ...
    //     },     
        ...
        exportTable(rows){
            ...
            self.download2(self.$server.newAllActivityStudyCardExport(parems));  
            //  window.open(url);          
        },   
        //追加课程和数量
        // newDailge(data,type){
        ...
        //             };
        //             self.$http.post(self.$server.getActivityStudyCardaddNum,parems).then(res=>{
        //                 if(res.status==200){
        //                     self.courseNumberState = false;
        //                     self.courseDivShowHide=null;
        //                     self.courseNumValue = null; 
        ...
        //                 };
        //                 self.$http.post(self.$server.getActivityStudyCardAddCourse,params).then(res=>{
        //                     if(res.status==200){
        //                         self.courseNumberState = false;
        //                         self.courseDivShowHide=null;
        //                         self.courseNumValue = null;  
        ...
        //     };
        //     self.$http.get(self.$server.getActivityStudyCardCourseList(params)).then(res=>{
        //         if(res.status==200){
        ...
        cancelBatchOk(rows){
            ...
            self.$http.post(self.$server.activityStudyCardCancel(params)).then(res=>{
               if(res.status==200){
        ...
        removeBatchOk(rows){
            ...
            self.$http.post(self.$server.activityStudyCardRemove(params)).then(res=>{
               if(res.status==200){
                    console.log(res);
                    var newData = []; 
        ...
        //创建学习卡
        creatlearnCard(){
            this.$router.push({path:"/marketelist/activitiesCard/creatActivitiesLearnCard"});
        },
        //状态筛选
        chooseVersionType(e){
            console.log("状态筛选",e);
        ...
        getStudylist(num){
            ...
            self.$http.get(self.$server.activityStudyCardList(params)).then(res=>{
               if(res.status==200){
                   var listIds = [];
        ...
        getStudylistSun(num){
            ...
            self.$http.get(self.$server.batchActivityStudyCardListInfo(params)).then(res=>{
               if(res.status==200){
                   var listIds = [];
                   var contentdate = res.content;
                   var nextPage = contentdate.nextPage;
        ...
        addTelePhoneNewId(){
            ...
            if(self.addTelephoneListCopy){
                if(self.addTelephoneListCopy.length>200){
                    ...
                    self.$http.post(self.$server.addCellphoneActivityStudyCard,params).then(res=>{
                        if(res.status==200){
