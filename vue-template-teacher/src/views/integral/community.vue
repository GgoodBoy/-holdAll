                        ...
                        <template slot-scope="scope">
                            <el-row> 
                                ...
                                <el-col :span="11" :offset="2">
                                    ...
                                    <el-button v-if="$authJudge('integral:source:forbidden')" v-show="scope.row.isForbidden!=1" type="text" class="black-btn lCenter-r" style="marginLeft:0" @click="channeRemoveBatch(scope.row)">禁用</el-button>
                                </el-col>
                            </el-row>                                                      
                        </template>
                    </el-table-column>                                                                                   
    ...
    methods:{
        ...
        isChannelOkEvenTwo(){
            ...
            switch (res.status) {
                case 200:
                        if(this.isStart==1){
                            this.$message.success('积分禁用成功')
                        }else{
                            this.$message.success('积分启用成功')
                        }
                        self.channelDelDialogVisible = false;
                        self.getStudylist(1);                       
                    break;
        ...
        getStudylist(num){
            ...
            self.$http.get(self.$server.communityList,query).then(res=>{
               if(res.status==200){
                   var listIds = [];
