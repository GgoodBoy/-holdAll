<template>
    <div class="two-level-tag" @click="hideSort">
        <div class="page-header lMarginBO30">
            <span class="page-title">APP少儿二级页标签设置</span>
            <el-button v-if="$authJudge('label:appchildrensecond:add')" type="primary" class="my-add-btn" @click="showDialog">添加</el-button>
        </div>
        <div class="my-table">
            <el-table
                border
                ...
                <el-table-column
                    ...
                    min-width="100">
                        <template slot-scope="scope">
                            ...
                            <div class="btn-box">
                                <el-button v-if="$authJudge('label:appchildrensecond:sort')" type="text" class="black-btn" @click.stop="sortRow(scope.row)">排序</el-button>
                            </div> 
                            <div class="btn-box">
                                <el-button v-if="$authJudge('label:appchildrensecond:del')" type="text" class="black-btn" @click="delRow(scope.row)">删除</el-button>
                            </div>                                              
                        </template>                    
                </el-table-column>                                                
            </el-table>
        ...
        <el-dialog :custom-class="'edit-dialog'" :visible.sync="editDialogVisible" :width="'500px'" :show-close="false">
            <div class="search-box">
                <el-input placeholder="输入标签名称查询" size="medium" clearable prefix-icon="el-icon-search" v-model="searchKey" maxlength="10" @keypress.native="pressEvent($event)"></el-input>
...
export default {
    ...
    methods:{
        getTableList(){
            ...
            this.$http.get(this.$server.getAppSecondPageLabelList,query).then(res=>{
                if(res.status==200){
                    this.loading = false;
                    this.tableData = res.content.labelListVos.list;
                    this.tableData.forEach((item)=>{
                        item.sortId = item.sort
                    })
                    this.labelIds = res.content.labelIds;
                    this.total = res.content.labelListVos.total;
                }
            })
        },
        showDialog(){
        ...
        dialogSure(){
            ...
            this.$http.post(`${this.$server.addAppSecondPageLabel}/2`,query).then(res=>{
                if(res.status==200){
        ...
        sortEvent(obj){
            let regExp = /^[0-9]+$/
                ...
                this.$http.post(`${this.$server.sortAppSecondPageLabel}/${obj.recordId}`,query).then(res=>{
                    if(res.status==200){
                        this.$message({
                            message: '排序成功！',
                            type: 'success'
        ...
        delRow(obj){
            this.$confirm('确定要删除这个标签吗？删除后APP相关二级页面将不再展示该标签', '删除确认', {
                ...
                this.$http.post(`${this.$server.delAppSecondPageLabel}/${obj.recordId}`,query).then(res=>{
                    if(res.status==200){
                        this.$message({
                            message: '删除成功！',
                            type: 'success'
