<template>
    <div class="module-edit goods">
        <div class="module-edit-header clearfix">
            ...
            <el-button v-if="$authJudge('goodsthings:goodspage:save')" type="primary" size="small" @click="saveData">保存</el-button>
        </div>
        <div class="module-edit-body">
            <div class="classify-box">
                <label>分类</label>
                <el-radio-group v-model="classifyShow">
...
export default {
    ...
    computed:{
        ...
        editData(){
            ...
            if(obj){
                ...
                return this.$store.state.goodsPage.moduleChildData.find(item=>item.localId==localId);
            }else{
                return []
            }
        },
    ...
    methods:{
        ...
        goodsSubModuleInfoList(id){
            ...
            this.$http.get(`${this.$server.goodsSubModuleInfoList}/${id}`).then(res=>{
                ...
                if(res.status==200){
                    ...
                    this.classifyShow = res.content.classifyShow;
                    let popularCoursesData = res.content.goodsPageDetailRecordList.map((item,index)=>{
                        return {
                            targetId:Number(item.goodsId),
                            goodsName:item.title
                        }
                    })
                    if(res.content.style==1){
                        this.courseCheckListStyle1 = res.content.goodsPageDetailRecordList.map(item=>{
                            return {
                        ...
                        this.courseCheckListStyle2 = res.content.goodsPageDetailRecordList.map(item=>{
                            return {
                                id:Number(item.goodsId),
                                title:item.title
                            }
        ...
        saveData(){
            ...
            // if(!regExp.exceptSpecialChar.test(this.editData.name)){
            //     this.$message.error('标题只能输入数字、字母、汉字');
            //     return;
            // }
            let query = {
                id:this.moduleId,
                moduleName:this.editData.name,
                subModuleType:2,
                style:this.styleVal,
            ...
            this.curEditData.forEach((item,index)=>{
                ...
                query['goodsPageDetailRecordList['+index+'].goodsId'] = item.targetId
                query['goodsPageDetailRecordList['+index+'].sort'] = index+1
            })
            let loadingInstance = this.$loading({
                target:document.querySelector('.module-edit')
            });
