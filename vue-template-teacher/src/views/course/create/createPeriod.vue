<template>
    <div class="creat-period-wrap create-period">
        <div class="form-wrap hide-required-asterisk">
            <h2 class="crumbs-list-common">
                <router-link to="/course">课程管理</router-link> / <router-link :to="`/course/manage/${$route.params['id']}?type=${$route.query['type']}`">课程详情</router-link> / {{this.periodId?'修改':'新增'}}课时
            </h2>
            <el-form ref="form"
                     :model="form"
                     :rules="rules"
                    ...
                    if (res.status === 200) {
                        ...
                        this.$message({
                            ...
                            onClose() {
                                ...
                                self.$router.push(
                                    `/course/manage/${self.courseId}?type=${
                                        self.inType
                                    }`
                                );
                            }
