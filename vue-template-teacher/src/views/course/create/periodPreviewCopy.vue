<template>
    <div class="period-wrap-copy preview-period">
        <div class="form-wrap hide-required-asterisk">
            <h2 class="crumbs-list-common">
                <router-link to="/courseCopy">课程管理</router-link> / <router-link :to="`/course/manageCopy/${$route.params['id']}?type=${$route.query['type']}`">课程详情</router-link> / 预览
            </h2>
            <el-form ref="form"
                     :model="form"
                     label-position="left"
                ...
                <el-button class="redBackColor butColorF lMarginTop40" style="margin-left:86px" @click="returnPat()">返回</el-button>
            </el-form>
        </div>
...
export default {
    ...
    mounted() {
        ...
        this.pid = this.$route.params["id"] || null;
        if (this.periodId) {
            this.getPeriodInfo();
        } else {
    ...
    methods: {
        returnPat(){
            this.$router.push(`/course/manageCopy/${this.$route.params['id']}?type=${this.$route.query['type']}`);
        },
        getPeriodInfo() {
            this.$http
