    ...
    <div class="matchIDBox">
        <div class="matchSelect">
            <el-row :gutter="10">
                ...
                <el-col :span="8">
                    <el-input v-model.trim="matchTitle" placeholder="请输入课程名称/课程ID查询（小鹅通/学国学网）"></el-input>                
                </el-col>
                <el-col :span="2">
        ...
        <div class="matchTableList">
            <div class="tableTitle">
                ...
                <p>该功能主要用于用户在小鹅通购买课程后，在我们APP中也可观看。</p>
            </div>
            <div>
                <el-table :data="data"
                        border
                    ...
                    <el-table-column label="小鹅通课程ID"
                                    align="center"
                                    prop="xiaoeProductId"
                                    min-width="150">
                    </el-table-column>
                    <el-table-column prop="xgxCourseId"
                                    label="学国学网课程ID"
                                    align="center"
                                    min-width="100">
                    </el-table-column>
            ...
            <div>
                <el-row >
                    <el-col :span="12" class="lLeft-r">
                        小鹅通课程ID
                        <el-tooltip class="item" effect="dark" content="小鹅通课程ID查看：进入专栏列表，点击某一个专栏进入专栏详情，此时查看浏览器地址栏后面有一串以p_开头的字符串，这就是课程ID（包含p_）" placement="top">
                ...
                <el-row class="lMarginTop20">
                    <el-col :span="10" class="lLeft-r"><el-input v-model.trim="matchXiaoeProductId" class="matchXiaoeProductId" @blur="changBackground(matchXiaoeProductId,'matchXiaoeProductId')" placeholder="请输入小鹅通课程ID"></el-input></el-col>
...
export default {
    ...
    methods: {
        ...
        matchingEven(types,data){
            switch (types) {
                ...
                case 'update':
                    ...
                    this.matchXiaoeProductId = data.xiaoeProductId;
                    this.matchXgxCourseId = data.xgxCourseId;
                    break;
                case 'delete':
                    this.updateId = data.id;
        ...
        getUpdataMatch(data,type){     
            ...
            var parems = {
                id:data.id||'',
                xiaoeProductId:data.xiaoeProductId,
                xgxCourseId:data.xgxCourseId
            };
            if(!self.disabledState){
                self.disabledState = !self.disabledState;
                this.$http.post(this.$server.courseRelationSave,parems).then(res=>{
                    if(res.status==200){
                            self.disabledState = !self.disabledState;
                            this.matchXiaoeProductId ='';
                            this.matchXgxCourseId = '';   
        ...
        getDeleteMatch(id){
            ...
            this.$http.delete(this.$server.courseRelationDelete(parems)).then(res=>{
               if(res.status==200){
                   this.dialogDeleveVisible = false;
        ...
        getMatchList(num){
            ...
            var parems = {
                keyWordId:self.matchTitle||'',
                pageSize:self.pageSize,
                pageNo:self.pageNo
            };
            self.$http.get(self.$server.courseRelationgGetPage(parems)).then(res=>{
               if(res.status==200){
                   var listIds = [];
