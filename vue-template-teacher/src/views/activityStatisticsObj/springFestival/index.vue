<template>
    <div class="page-wrap activity-springFestival-page">
        ...
        <div class="page-head">
            ...
            <div class="flexBox">
                <p>活动总访问次数</p>
                <p>
                    <span>{{baseData.totalVisitNum||0}}</span>次
                </p>
                <p>今日访问：{{baseData.todayVisitNum||0}} 次</p>                
            </div>
            <div class="flexBox">
                <p>活动总注册人数</p>
                <p>
                    <span>{{baseData.totalRegisterNum||0}}</span>人
                </p>
                <p>今日注册：{{baseData.todayRegisterNum||0}} 人</p>                
            </div>
        </div>
...
import breadcrumb from "@/components/breadcrumbGX/breadcrumb";
    export default{
        ...
        methods:{
            ...
            getActivityBaseData(){
                var self = this;            
                self.$http.get(self.$server.getFuBaoActivityBaseData).then(res=>{
                    if(res.status==200){
            ...
            getTableInfoList(num){
                ...
                self.loading = true;              
                self.$http.get(self.$server.getFuBaoListByChannel).then(res=>{
                    if(res.status==200){
                        self.loading = false;
