<template>
    <div class="page-wrap activity-statis-page">
...
import breadcrumb from "@/components/breadcrumbGX/breadcrumb";
    export default{
        ...
        methods:{
            ...
            getActivityBaseData(){
                var self = this;            
                self.$http.get(self.$server.getActivityBaseDataRd).then(res=>{
                    if(res.status==200){
            ...
            allotRole(data){
                this.$router.push({
                    path:`/activityStatistics/worldBookDay/dataDetail/1/${data.type}`,                   
                });   
            },
            getTableInfoList(){
                var self = this;
                self.loading = true;              
                self.$http.get(self.$server.getListByChannelRd).then(res=>{
                    if(res.status==200){
                        self.loading = false;
