<template>
    <div>
        ...
        <el-row>
            ...
            <el-col :span="18" class="lCenter-r lMarginTop20" v-if="$authJudge('set:profile:release')">
                <el-button class="redBackColor butColorF" @click="submitForm(1)">发布</el-button>                
            </el-col>            
        </el-row>
    </div>
</template>
...
export default {
    ...
    methods: {
            ...
            submitForm(num){
                ...
                //     return ;
                // }    
                if(self.introduce.replace(/\s+/g, "").replace(/<[^<>]+>/g,'').length>50000){
                    this.$message.error("内容长度在 1 到 50000 个字符");
                    return ;
                }                              
