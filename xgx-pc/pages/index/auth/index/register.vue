<template>
    <div class="register-page" @keydown="pressKeyDown($event)">
        <div class="page-h clearfix">
            <div class="l">
            </div>
            <div class="r">
                <span @click="$router.push('/auth/login')">已有账号登录</span>
            </div>
        </div>
        <p class="title">注册学国学网</p>
        <p class="tips" v-show="fromLogin">亲爱的用户，您当前输入的帐户还未注册，注册仅需1分钟即可完成</p>
        <div class="block">
            <MyBox :className="'phone'">
                <div slot="content">
                    <i></i>
                    <input class="my-input" type="text" placeholder="手机号" v-model="cellphone"  maxlength="11" autocomplete="off"/>
                </div>
            </MyBox>
        </div>
        <div class="block clearfix">
            <MyBox :className="'img-code medium'">
                <input class="my-input" type="text" placeholder="请输入右侧图形验证码" slot="content" v-model="imgCode" maxlength="6" autocomplete="off" />
            </MyBox>
            <img :src="`https://${imgCodeUrl}`" alt="图形验证码" class="img-code-box" @click="getImgCode"/>
        </div>
        <div class="block clearfix">
            <MyBox :className="'sms-code medium'">
                <input  class="my-input" type="text" placeholder="收到的验证码" slot="content" v-model="code" maxlength="6" autocomplete="off" />
            </MyBox>
        ...
        <div class="block">
            <MyBox :className="'pwd'" >
                <input class="my-input" type="password" placeholder="密码由7-20位字母或数字组成" slot="content" v-model="password"  maxlength="20" autocomplete="off" />
            </MyBox>
        </div>
        <div class="block">
            <MyBox :className="'re-pwd'">
                <input class="my-input" type="password" placeholder="重复密码" slot="content" v-model="rePassword"  maxlength="20" autocomplete="off"/>
            </MyBox>
        </div>
        ...
        <div class="block register-btn-box">
            <button class="primary-btn" @click="register">注册</button>
        </div>
        <div class="block error-tips" >
            <div v-show="errorText.length>0">
                <i></i>
                <p>{{errorText}}</p>
            </div>
        </div>
...
@Component({
    ...
    async asyncData({app,query}:NuxtContext):Promise<any>{
        ...
        return {
            ...
            fromLogin:cellphone?true:false
        }
    },
    computed:{
        smsBtnState(){
    ...
    methods:{
        ...mapActions(['setUserData','setInterestDialog','setReceiveCouponClick'])
    },
})
export default class Auth extends Vue{
    ...
    async sendSms(){
        ...
        let res = await this.$axios.$get(this.$server.verifyCellphone,query)
        if(res.status==200){
            let {state} = res.content;
            if(state==2){
                this.errorText = '当前手机号被禁用';
                return; 
            }else if(state==3){
                this.errorText = '当前手机号已注册';
                return;   
            }
        }
        if(this.imgCode==''){
    ...
    async register(){
        ...
        if(this.password!=this.rePassword){
            this.errorText = '两次密码输入不一致，请重新输入';
            return;
        }
        if(!this.iconCheck){
        ...
        let loadingInstance = this.$loading.service({
            target:document.getElementsByClassName('register-page')[0],
        ...
        let res = await this.$axios.$post(this.$server.signUp,query)
