<template>
    <div class="free-box">
        ...
        <div class="course-center" v-loading="loading">
            <div class="courseListBox" v-for="(item,index) in courseListData" :key="index">
                    ...
                    <div class="divCenter">
                        ...
                        <div class="uniteCss imgUniteCss">
                            <el-row>
                                ...
                                <el-col v-if="item.lectureTeacherList.length>1">
                                    <div class="imgBoxDiv">
                                        <div class="imgBoxOne" v-bind:style="{ 'z-index':indexs+1, 'left': (indexs)*(120/(item.lectureTeacherList.length>15?15:item.lectureTeacherList.length))+'px' }" v-for="(items,indexs) in item.lectureTeacherList" :key="indexs">
                ...
                <!-- <div class="divCourseThree">
                    <div class="threeSonDiv">
                        <el-progress :percentage="Number(item.progress.substr(0,item.progress.length-1))"></el-progress>
                    </div>
                </div> -->
                <div class="divCourseFour">
                    <div v-if="Number(item.progress.substr(0,item.progress.length-1))==100" @click="learnCourseEven(item)">再学一次</div>
                    <!-- <div v-if="Number(item.progress.substr(0,item.progress.length-1))<100&&Number(item.progress.substr(0,item.progress.length-1))>0" @click="learnCourseEven(item)">继续学习</div> -->
                    <div v-if="Number(item.progress.substr(0,item.progress.length-1))<100&&Number(item.progress.substr(0,item.progress.length-1))>0" @click="learnCourseEven(item)">开始学习</div>
                    <div v-if="Number(item.progress.substr(0,item.progress.length-1))==0" @click="learnCourseEven(item)">开始学习</div>
                </div>
            </div>
            <div v-show="listNumber==0" class="noDataCss">您还没有看过免费课程</div>
        </div>
        <div class="pageCss">
